...
$.ajax({
    url: 'http://localhost:5555/chatTestBBB/vendor/js/optionSelect.json',
    dataType: 'json'
}).done(function (res) {
        optionUp(res);
    })
...
function selectOption(data, that) {
    ...
    var trend_html = '<div class="bubble robot"><div class="img"><img src="../vendor/img/ml_1272.png"></div><p>以下是為您查詢的結果</p></div>'
...
function optionUp(data) {
    ...
    $('.opctionList').on('click', 'a', function () {
        var that = this;
        $.ajax({
            url: 'http://localhost:5555/chatTestBBB/vendor/js/register.json',
            dataType: 'json'
        })
            .done(function (res) {
                //alert(res)
