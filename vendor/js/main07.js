...
function sendAnwser(inputText) {
    $.ajax({
        type:'POST',
        url:'http://192.168.1.184:8081/api/query',
        data:{
        ...
        if (parseData.Argument[i].Type == "Text") {
            ...
            if (parseData.Argument[i].PayloadInfo == "ListSeatAvailableArray") {
                ...
                for (var x = 0; x < ListSeatAvailable.length; x++) {
                    var registerBox = '<div class="registerBox"><div class="registerTop">'
                    registerBox += '<div class="date">' + ListSeatAvailable[x].FlightInfo.DepartDate + '</div>'
                    ...
                    registerBox += '<div class="location-s"><span>' + ListSeatAvailable[x].FlightInfo.DepartFrom.slice(0, -5) + '</span><i>-</i><span>' + ListSeatAvailable[x].FlightInfo.ArriveTo.slice(0, -5) + '</span></div>'
                    registerBox += '<div class="info">'
                    registerBox += '<span class="num">' + ListSeatAvailable[x].FlightInfo.AirlineCodeFlightNo + '</span>'
                    registerBox += '<div class="time"><span>' + ListSeatAvailable[x].FlightInfo.DepartTime + '</span><i>-</i><span>' + ListSeatAvailable[x].FlightInfo.ArriveTime + '</span></div>'
                    registerBox += '</div></div>'
                    registerBox += '<div class="registerBottom">'
                    var block = '';
...
function init(){
    ...
    $('.xBtn').on('click', function () {
        $('.chatBot').removeClass('on').addClass('off')
        setTimeout(function(){
            $('.chatBot').remove()
        },2000)
    ...
    $(document).off('click').on('click', '.checkCube i', function() {
        $(this).toggleClass('on')
    ...
    $(document).on('mouseenter mouseleave', '.selectBtn .inner a',function(){
...
$(window).load(function () {
    ...
    $('.orderBtn').on('click', function() {
        if($(".chatBot#chatBot").length === 0){
            var chatBox ='<div class="chatBot" id="chatBot"><div class="title"><div class="inn">訂票小幫手<i class="fa fa-minus minus" aria-hidden="true"></i><i class="fa fa-times xBtn" aria-hidden="true"></i></div></div><div class="messages"><div class="messages-content"></div></div><div class="messagesBox"><textarea class="msg-input textHolder" placeholder="請輸入您的問題..." rows="1"></textarea><button class="msg-send">送出</button></div></div>';
            $(chatBox).prependTo('body').addClass('on');
            init();
            welcome();
        }
